...
import type { Role, School } from "../../shared/types";
...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: "/parents", label: "أولياء الأمور", icon: "parents", roles: ADMINS },
...
export default function Layout() {
  ...
  const schoolName = schoolData?.school.name ?? "منصة المدرسة";
  ...
  return (
    <div className="min-h-dvh bg-slate-100">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 md:hidden">
        ...
        <span className="truncate px-2 text-base font-bold text-teal-800">{schoolName}</span>
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute inset-y-0 right-0 w-72 overflow-y-auto bg-white shadow-xl">
            <div className="border-b border-slate-200 p-4">
              <p className="text-base font-bold leading-tight text-teal-800">{schoolName}</p>
              <p className="mt-2 text-sm font-medium text-slate-700">{user.full_name}</p>
      ...
      <div className="md:flex">
        ...
        <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 md:border-l md:border-slate-200 md:bg-white">
          <div className="border-b border-slate-200 p-4">
            <p className="text-lg font-bold leading-tight text-teal-800">{schoolName}</p>
            <p className="mt-2 text-sm font-medium text-slate-700">{user.full_name}</p>
            <p className="text-xs text-slate-500">{ROLE_LABELS[user.role]}</p>
          </div>
